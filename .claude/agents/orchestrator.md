...
When a subagent finishes a long-running task and the editor may have wandered off, ping their phone so they don't discover the completed step hours later. You are the only agent with `Bash` — you own this.

**Transport:** `~/bin/ntfy-send.sh` (note: `ntfy-send.sh`, not `ntfy.sh`). Topic is configured in `~/.config/ntfy/topic`. Usage:
...
**When to send (the trigger):**

Use subagent elapsed time as the signal for "the editor probably walked away." Before launching any subagent, capture a start timestamp:
...
1. `ELAPSED >= ${WRITERS_ROOM_NTFY_THRESHOLD_SEC:-180}` (default 3 minutes — few humans sit watching a spinner for three minutes), **OR**
2. The subagent failed / returned an error, **OR**
3. The agent completed a step that now requires the editor's approval and the next step is gated on them (always notify on safety-reviewer findings that need human review).

Short steps that return inside the threshold (quick cache reads, small edits, a fast trimmer pass) don't ping — no notification spam.

Configuration via env:

- `WRITERS_ROOM_NTFY_ENABLED` — set to `0` or `false` to disable all pings (default on)
- `WRITERS_ROOM_NTFY_THRESHOLD_SEC` — override the 3-minute threshold

If the script is missing, the topic file is missing, or the POST fails, log a one-line warning and continue — never block the pipeline on notification.

**Message conventions:**

- **Title:** `[<profile-id>] <agent-role> done: <slug>` — e.g. `[example-blog] drafter done: my-article-slug`
- **Message:** one line. What changed + what the editor needs to decide next. Under 120 chars if possible.
- **Priority:**
  - `default` — step completed cleanly, next step gated on editor
  - `high` — safety-reviewer flagged something, or step produced unexpected output that needs attention
  - `urgent` — subagent failed or cache/Obsidian sync failed
  - `low` — purely informational (rare — prefer not sending at all)
- **Tags:** profile id + agent role, e.g. `example-blog,drafter` or `example-newsletter,safety`. Tags render as emoji/labels on the receiver.

**Examples:**

```bash
# Normal completion, gated on editor
~/bin/ntfy-send.sh \
  --priority default \
  --title "[example-blog] structure-analyst done: my-article" \
  --tags example-blog,structure \
  "Structural analysis ready. Thesis tracks; 1 coverage gap flagged. Say go for rhythm."

# Safety reviewer flagged something
~/bin/ntfy-send.sh \
  --priority high \
  --title "[example-blog] safety flagged: my-article" \
  --tags example-blog,safety,warning \
  "Defamation risk on para 4; factual claim needs sourcing. Review before publish."

# Failure
~/bin/ntfy-send.sh \
  --priority urgent \
  --title "[example-blog] drafter FAILED: my-article" \
  --tags example-blog,error \
  "Subagent returned no draft.md. Cache and Obsidian unchanged. Need eyes."
```

**Don't notify for:** profile switches, reading files, single cache commits, or anything the editor triggered inside the same minute (they're clearly at the desk).
...
- Updating todo.md
- Sending ntfy notifications on subagent completion (see "Phone Notifications" above)
